...
import { renderPage, translatableStrings } from '../src/templates.mjs';
...
const templateHtml = readFileSync(join(ROOT, 'index.html'), 'utf8');
const probe = parseHTML(templateHtml).document;
const LANGS = [...probe.querySelectorAll('#lang-select option')].map((o) => o.value);

// Same source of truth as the build — see src/registry.mjs.
const REGISTRY = buildRegistry(ROOT, probe, templateHtml);
const PAGES = REGISTRY.map((p) => p.id);
...
// Two independent code paths over the same data — the renderer tags elements,
// the translator enumerates strings. If they drift, a page silently stops
// being translatable, which nothing else here would catch.
for (const page of REGISTRY) {
  if (page.authored) continue;
  ...
  const declared = translatableStrings(page).map((s) => s.key).filter((k) => k.startsWith('page.'));
  declared.forEach((k) =>
...
for (const lang of LANGS) {
  for (const id of PAGES) {
    ...
    // Arabic is marked right-to-left per text element, not on <html>: the
    // layout (sidebar, nav, grids) is not RTL-aware, so flipping the whole
    // document would break it. Assert what the site actually does.
    if (lang === 'ar') {
      const rtlNodes = [...document.querySelectorAll('[dir="rtl"]')];
      check(rtlNodes.length > 0, `${where}: no element marked dir=rtl`);
    }
